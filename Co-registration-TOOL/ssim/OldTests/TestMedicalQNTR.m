...
directory = ['TestsMM/',interp_type,num2str(nrefine),'-',cost_function];
...
nome_template = ['Images/','medical2',num2str(2^j),'.jpg']; 
nome_reference = ['Images/','medical1',num2str(2^j),'.jpg']; 
...
options=optimset('GradObj','on',...%'HessUpdate','steepdesc',...
    'MaxIter',maxit,'DerivativeCheck',der_control,...
     'TolFun',1.e-4,'TolX',1.e-4,...  %'DiffMinChange',1.e-4,
...
disp(['Percentuale QN - TR = ',num2str((t1-t0)/(t2-t0)),' - ',num2str((t2 - t1)/(t2 - t0))]);
