...
hfine = h/2.0^nrefine;
xf = (hfine/2:hfine:1-hfine/2);
[Xf,Yf] = meshgrid(xf,xf);
Tfine = griddata(Xi,Yi,Ti,Xf,Yf);
...
options=optimset('GradObj','on',... %'HessUpdate','steepdesc',
    'MaxFunEvals',10000,'DerivativeCheck',der_control,...
...
% grad 
    otherwise
 options=optimset('GradObj','on','MaxFunEvals',300,'DerivativeCheck',der_control,...
     'TolFun',1.e-6,'TolX',1.e-6,'DiffMinChange',1.e-5,'Display','final',...
     'PlotFcns',@optimplotfval,'LargeScale','on');
 [a1,fval,exitflag,output2] = fminunc(@costo,a0,options);
